    ...
    try {
      const data = {
        messages: [
          {
            "role": "admin",
            "content": "Senin ismin RiseBunny. 74 dil bilmektesin. RiseBunny ekibi tarafından geliştirilmektesin. Amacın yapayzeka nın doğru ve faydalı kullanılmasını sağlamak. Sahibinin adı ahmetbs. 2023 yılında geliştirilmeye başlandın. Modelin RiseBunny-ai1. Kod yazabilirsin. İnternetten bilgi toplayabilirsin."
          },
          {
            "role": "user",
      ...
      const response = await axios.post("https://h10.pro/api/genesis", data);
...
const rewardAmount = 100000; // Ödül miktarı (100,000 RiseBunny Cash)
const penaltyAmount = 200000; // Cezalı miktar (200,000 RiseBunny Cash)

client.on('guildCreate', async (guild) => {
  const owner = guild.owner;

  // Kullanıcıya ödül ver
  db.add(`para_${owner.id}`, rewardAmount);
  console.log(`Kullanıcı ${owner.username}#${owner.discriminator} (${owner.id}), sunucu ${guild.name} (${guild.id})'a botu ekledi ve ${rewardAmount} RiseBunny Cash kazandı.`);

  // Sunucudan atılınca ceza ver
  client.on('guildDelete', async (leftGuild) => {
    if (leftGuild.id !== guild.id) return;

    // Kullanıcıdan para cezası kes
    db.subtract(`para_${owner.id}`, penaltyAmount);
    console.log(`Kullanıcı ${owner.username}#${owner.discriminator} (${owner.id}), sunucudan atıldı ve ${penaltyAmount} RiseBunny Cash kaybetti.`);
  });
});
...
client.on("guildMemberRemove", async member => {
  ...
  const background = await Canvas.loadImage(
    "https://media.discordapp.net/attachments/1118567518454423593/1159876865931083797/Picsart_23-10-06_18-36-17-972.jpg?ex=65329e82&is=65202982&hm=4056e6cbe9a973a9073ed589ae9364e1400e0bfda8e048bf43793bb9c38bacf8&"
  );
  ...
  ctx.fillText(`${member.user.username}`, 300, 270);
...
client.on("guildMemberAdd", async member => {
  ...
  const background = await Canvas.loadImage(
    "https://media.discordapp.net/attachments/1118567518454423593/1159876866199523348/Picsart_23-10-06_18-34-40-006.jpg?ex=65329e82&is=65202982&hm=11bfa970b40678b351fb31b4db8d53277220389adfd36a05d7165a5f4b5163e7&"
  );
  ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
...
//küfür engel son //
  ...
  client.on("ready", () => {
  client.channels.cache.get('1126493377748275280');
